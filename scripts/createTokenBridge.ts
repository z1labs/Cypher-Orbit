...
async function getNativeToken({
  ...
  const rollupContract = RollupAdminLogic__factory.connect(
    rollup,
    provider as any // Casting to 'any' to bypass the type error
  );
  const bridge = await rollupContract.bridge();

  try {
    const bridgeContract = IERC20Bridge__factory.connect(bridge, provider as any);
    return (await bridgeContract.nativeToken()) as `0x${string}`;
  } catch (error) {
...
export const createNewTokenBridge = async (
  ...
  // Convert chain IDs from bigint to number
  const l1ChainId = Number(l1NetworkInfo.chainId);
  const l2ChainId = Number(l2NetworkInfo.chainId);

  const deployer = privateKeyToAccount(sanitizePrivateKey(baseChainDeployerKey));
  const rollup = RollupAdminLogic__factory.connect(
    rollupAddress,
    l1Provider as any // Casting to 'any' to bypass the type error
  );
  ...
  if (nativeToken !== ZeroAddress) {
    ...
    const allowanceParams = {
      nativeToken,
      owner: deployer.address as Address,
  ...
  const txRequest = await createTokenBridgePrepareTransactionRequest({
    params: {
      ...
      rollupOwner: deployer.address as Address,
    },
    parentChainPublicClient,
    orbitChainPublicClient,
    account: deployer.address as Address,
  });
  ...
  const l1Network: L1Network = {
    blockTime: 10,
    chainID: l1ChainId, // Use converted number
    explorerUrl: '',
    isCustom: true,
    name: l1NetworkInfo.name,
    partnerChainIDs: [l2ChainId], // Use converted number
    isArbitrum: false,
  };

  const l2Network: L2Network = {
    chainID: l2ChainId, // Use converted number
    confirmPeriodBlocks: Number(await rollup.confirmPeriodBlocks()),
    ethBridge: {
      bridge: coreContracts.bridge,
      inbox: coreContracts.inbox,
      outbox: coreContracts.outbox,
      rollup: rollup.address as Address,
    ...
    isCustom: true,
    name: 'OrbitChain',
    partnerChainID: l1ChainId, // Use converted number
    partnerChainIDs: [],
